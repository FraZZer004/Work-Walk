...
struct AnalysisView: View {
    ...
    // États de données calculées (Totaux Moyens)
    ...
    @State private var totalWorkFlights: Double = 0; @State private var totalLifeFlights: Double = 0 // 👈 AJOUTÉ
    ...
    @State private var historyFlights: [DailyData] = [] // 👈 AJOUTÉ
                    ...
                    VStack(alignment: .leading, spacing: 15) {
                        ...
                        VStack(spacing: 10) {
                            ...
                            if !isSelectedDateCurrentWeek() {
                                Button(action: {
                                    ...
                                    withAnimation { selectedDate = Date(); analyzeWeek() }
                                }) {
                                    HStack(spacing: 6) { Image(systemName: "arrow.counterclockwise"); Text("Revenir à la semaine en cours") }
                                        .font(.caption).bold().foregroundStyle(.orange).padding(.vertical, 6).padding(.horizontal, 12).background(Color.orange.opacity(0.1)).clipShape(Capsule())
                                }
                        ...
                        if isLoading {
                            ...
                            VStack(spacing: 12) {
                                ...
                                SimpleScoreCard(type: .flights, workVal: totalWorkFlights, lifeVal: totalLifeFlights).onTapGesture { selectedMetric = .flights } // 👈 AJOUTÉ
    ...
    // 👇 SWITCH MIS À JOUR (Correction de l'erreur)
    func getDataFor(_ type: MetricType) -> [DailyData] {
        ...
        case .flights: return historyFlights // 👈 AJOUTÉ
        }
    }
    ...
    func analyzeWeek() {
        ...
        group.notify(queue: .main) {
            withAnimation {
                ...
                self.totalWorkFlights = tWFlights/7; self.totalLifeFlights = tLFlights/7 // 👈 Moyenne
...
struct SimpleScoreCard: View {
    ...
    func iconFor(_ type: MetricType) -> String { switch type { case .steps: return "figure.walk"; case .calories: return "flame.fill"; case .distance: return "map.fill"; case .heart: return "heart.fill"; case .flights: return "figure.stairs" } } // 👈 AJOUTÉ ICI
}
