...
struct HealthProfileView: View {
    ...
    var body: some View {
        NavigationStack {
            // ZStack retirée pour laisser le système gérer le fond (Blanc/Noir)
            ScrollView {
                VStack(spacing: 25) {
                    ...
                    VStack(spacing: 10) {
                        Text("VOTRE IMC ACTUEL")
                            .font(.caption).bold().foregroundStyle(.secondary) // S'adapte auto
                            .tracking(2)
                        
                        HStack(alignment: .lastTextBaseline) {
                            Text(String(format: "%.1f", bmi))
                                .font(.system(size: 60, weight: .heavy, design: .rounded))
                                .foregroundStyle(.primary) // Noir en clair, Blanc en sombre
                        ...
                        GeometryReader { geo in
                            ZStack(alignment: .leading) {
                                ...
                                Circle()
                                    // Le curseur s'adapte aussi (blanc en sombre, un peu grisé en clair pour contraste ou inversé)
                                    // Ici on garde .white avec une ombre forte pour qu'il ressorte sur la couleur
                                    .fill(.white)
            ...
            // On retire les forçages de couleurs de la toolbar pour laisser faire le système
        }
    }
    
    // Fonction d'aide pour calculer l'offset
...
struct GenderButton: View {
    ...
    var body: some View {
        Button(action: action) {
            ...
            .frame(height: 100)
            // Fond adaptatif : Orange si sélectionné, sinon gris système (clair ou sombre)
        ...
        HStack {
            ...
            VStack(alignment: .leading, spacing: 2) {
                ...
                HStack(alignment: .firstTextBaseline, spacing: 2) {
                    Text(value).font(.title2).bold().foregroundStyle(.primary) // Auto Noir/Blanc
                    Text(unit).font(.subheadline).foregroundStyle(.secondary)
                }
            }
            Spacer()
            stepper().scaleEffect(1.1)
        }
        .padding()
        // Fond adaptatif
        .background(Color(UIColor.secondarySystemBackground))
        .cornerRadius(20)
        // Bordure subtile en mode clair comme sombre
        .overlay(
            RoundedRectangle(cornerRadius: 20)
